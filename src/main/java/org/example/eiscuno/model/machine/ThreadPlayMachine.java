...
public class ThreadPlayMachine extends Thread {
    ...
    public void run() {
        while (true) {
            if (hasPlayerPlayed) {
                ...
                gameUnoController.muestraMazoMaquina();
                ...
                if (!table.getCurrentCardOnTheTable().getIsSpecial()) {
                    ...
                    for (int i = 0; i < cardsPlayerTemporal.size(); i++) {
                        ...
                        if (card.getPath().contains("reserve_")) {
                            ...
                            newCard = tomaCartasMazo();
                            ponerMazoTabla(newCard, i);
                        } else if (card.getPath().contains("2_wild_draw")) {
                            if (colorOrValue(card)) {
                                ponerMazoTabla(card, i);
    ...
    private void validateBasicCards() {
        ...
        for (Card card : cardsPlayerTemporal) {
            if (!card.getIsSpecial() && (colorOrValue(card))) {
                ponerMazoTabla(card, index);
                flag = true;
                break;
            } else if (card.getPath().contains("skip_")) {
                if (colorOrValue(card)) {
                    siguiente(card, index);
                    flag = true;
                    break;
                }
    ...
    private void ponerMazoTabla(Card card, int index) {
        table.addCardOnTheTable(card);
        tableImageView.setImage(card.getImage());
    ...
    private Card tomaCartasMazo() {
        while (deck.getIsThereCardsOnDeck()) {
            System.out.println(deck.getIsThereCardsOnDeck());
            machinePlayer.addCard(deck.tomaCarta());
            Card newCard = ultimaCartaAgregada();
    ...
    private void siguiente(Card card, int index) {
        ponerMazoTabla(card, index);
    ...
    private Card ultimaCartaAgregada() {
